...
| 项目 | 值 |
|------|-----|
| OS | Ubuntu 24.04.4 LTS (x64) (WSL) |
| Node.js | v24.16.0，koffi 正常 |
| Bun | v1.3.14 |
| Deno | 2.8.1 |
| KossJS | v0.1.0-dev.6 |
| 测试框架 | Vitest v4.1.7 / Bun test / Deno test |
| Rust 测试库 | ***test-lib/target/release/libsenri_test.so***（已复制到 ***test-lib/release/senri_test.dll*** ） |
| KossJS 运行时 | ***src/__tests__/kossjs.so*** （未公布的 v0.1.0-dev.6 版本） |
...
### 1. ***normalize.test.ts*** — 12 ✓ / 0 ❌ / 0 PASS
...
| ***add_float*** | ✓ |
| ***is_positive*** | ✓ |
| ***string_length*** | PASS（Bun cstring crash on all platforms） |
| ***string_equals*** | PASS（Bun cstring crash on all platforms） |
| ***test_bigint_ops*** | ✓ |
| ***funcAsync basic*** | ✓ |
| ***funcAsync string*** | PASS（Bun cstring crash on all platforms） |
